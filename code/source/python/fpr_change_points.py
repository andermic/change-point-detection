...
for fpr in FPR_LIST:
    ...
    for file_num in range(len(score_files)):
        ...
        cur_scores = sorted(cur_scores_pre, key=lambda x:x[1], reverse=True)
        print 'done sorting\n'
        cur_fps = 0
        cps = []
        if DATASET == 'osu_hip':
            ...
            for score in cur_scores:
                changed_index = (score[0] + KPRE) / 3600
            ...
            for score in cur_scores:
                cur_score = score[0] + KPRE + 1  #Add 1 to compensate for MATLAB's 1-based indexing
                try:
                    changed_index = binary_search(event_ticks, cur_score, 0, len(event_ticks)-2)
                except RuntimeError:
        ...
        fstream = open('%s/%s' % (fpr_path, score_files[file_num].split('_')[-1]), 'w')
